...
def generate_evaluation(transcript, test, job_role, skills_to_rate):
    ...
    def summarize_text(text, max_chunk_size=5000):
        ...
        for chunk in chunks:
            ...
            summary_ids = model.generate(
                inputs, max_length=130, min_length=30, length_penalty=2.0, num_beams=4, early_stopping=True)
    ...
    # Summarize & filter
    summarized_transcript = summarize_text(
        extract_relevant_lines(transcript, skills_to_rate))
    summarized_test = summarize_text(
        extract_relevant_lines(test, skills_to_rate))
